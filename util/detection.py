...
from util.align_dlib_new import AlignDlib
from util.symbol.resnet import *
from util.symbol.config import config
from util.symbol.processing import bbox_pred, clip_boxes, nms
...
def detect(fl, args):
    ...
    for i in range(dets.shape[0]):
        bbox = dets[i, :4]
        # crop_img = color[int(round(bbox[1] / scale)):int(round(bbox[3] / scale)),
        #            int(round(bbox[0] / scale)):int(round(bbox[2] / scale))]
        # bb =[(467, 239) (529, 301)]
        # print((int(round(bbox[0]/scale))))
        # print((int(round(bbox[1]/scale))))
        # print((int(round(bbox[2] / scale))))
        # print((int(round(bbox[3] / scale))))

        # bb = [(int(round(bbox[0]/scale)), int(round(bbox[1]/scale))), (int(round(bbox[2]/scale)), int(round(bbox[3]/scale)))]
        bb = dlib.rectangle(int(round(bbox[0]/scale)), int(round(bbox[1]/scale)), int(round(bbox[2]/scale)), int(round(bbox[3]/scale)))
        # bb = [(849, 1466, 1404, 2021)]
        ...
        # resized_image = cv2.resize(crop_img, (180, 180))

        dirname = os.getcwd()
        image_output_dir = os.path.join(dirname, "output")
