...
class AlignDlib:
    ...
    def align(self, imgDim, rgbImg, bbarray=None, pad=None, ts=None,
              landmarks=None, landmarkIndices=INNER_EYES_AND_BOTTOM_LIP,
              opencv_det=False, opencv_model="../model/opencv/cascade.xml",
              only_crop=False):
        r"""align(imgDim, rgbImg, bbarray=None, landmarks=None, landmarkIndices=INNER_EYES_AND_BOTTOM_LIP)
        ...
        :param rgbImg: RGB image to process. Shape: (height, width, 3)
        :type rgbImg: numpy.ndarray
        :param bbarray: Bounding box around all the faces to align. \
        :type bbarray: array(dlib.rectangle)
        ...
        if bbarray is None:
            if opencv_det:
                ...
                bbarray = self.getAllFaceBoundingBoxes(rgbImg)
        ...
        if landmarks is None:
            for bb in finalbb:
                ...
                # landmarksarr.append(self,landmarks)
                npLandmarks = np.float32(landmarks)
                # print (npLandmarks)
                npLandmarksarr.append(npLandmarks)
        ...
        if not only_crop:
            outimgarr = []
            for npLandmarks in npLandmarksarr :
                H = cv2.getAffineTransform(npLandmarks[npLandmarkIndices], dstLandmarks)
                outimg = cv2.warpAffine(rgbImg, H, (imgDim, imgDim))
                outimgarr.append(outimg)
            return outimgarr
        else:
